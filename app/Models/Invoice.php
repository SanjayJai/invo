...
class Invoice extends Model
{
    ...
    public static function generateUniqueInvoiceId(): string
    {
        ...
        if ($latestInvoice) {
            $lastInvoiceNumber = intval(substr($latestInvoice->invoice_id, strlen($prefix))); // Extract numeric part
            $nextInvoiceNumber = $lastInvoiceNumber + 1;
        } else {
            $nextInvoiceNumber = 1; // Start from 1 if no invoices exist yet
        }
    
        // Format the invoice number with prefix and padding
        $invoiceId = $prefix . str_pad($nextInvoiceNumber, 3, '0', STR_PAD_LEFT); // Assuming max invoice number length is 6 digits
