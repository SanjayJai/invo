...
window.addEventListener("turbo:load", (event) => {
    $(
        'input:text:not([readonly="readonly"]):not([name="search"]):not(.front-input):not([id="time_range"]):not(.removeFocus)'
    )
        .first()
        .focus();
});
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                let date = moment(invoice.invoice_date).format("DD/MM/YYYY");
                ...
                let whatsappLink = `"https://wa.me/+919688772360?text=Hello" *${name}*,%0a%0aThank you for doing business with *${appName}*.%0aPlease find your invoice details below.%0a%0aInvoice No: ${invoiceNo}%0aInvoice Date: ${date}%0aDue Date: ${dueDate}%0aTotal Amount: ${totalAmount}%0aPaid Amount: ${paidAmount.toFixed(
                    2
