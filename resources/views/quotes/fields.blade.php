<div class="row">
    ...
    <div class="col-lg-4 col-sm-12 mb-5">
        ...
        {{ Form::text('quote_date', null, ['class' => 'form-control', 'id' => 'quote_date', 'autocomplete' => 'off', 'required']) }}
    </div>
    <div class="mb-5 col-lg-4 col-sm-12">
        {{ Form::label('due_date', __('messages.quote.due_date') . ':', ['class' => 'form-label required mb-3']) }}
        {{ Form::text('due_date', null, ['class' => 'form-control', 'id' => 'quoteDueDate', 'autocomplete' => 'off', 'required']) }}
    </div>
            ...
            <table class="table table-striped box-shadow-none mt-4" id="billTbl">
                ...
                <tbody class="quote-item-container">
                    <tr class="tax-tr">
                        ...
                        <td class="table__qty">
                            {{ Form::number('quantity[]', null, ['class' => 'form-control qty-quote', 'required', 'min' => 1, 'oninput' => "validity.valid||(value=value.replace(/\D+/g, ''))"]) }}
                        </td>
                        <td>
                            {{ Form::number('price[]', null, ['class' => 'form-control price-input price-quote', 'oninput' => "validity.valid||(value=value.replace(/[e\+\-]/gi,''))", 'min' => '0', 'step' => '.01', 'pattern' => "^\d*(\.\d{0,2})?$", 'required', 'onkeypress' => 'if(this.value.length==8) return false;']) }}
                        </td>
    ...
    <div class="row mt-4">
        <div class="col-lg-7 col-sm-12 mt-2 mt-lg-0 align-right-for-full-screen">
            <div class="mb-2 col-xl-6 col-lg-10 col-sm-12 float-lg-end">
                {{ Form::label('discount', __('messages.invoice.discount') . ':', ['class' => 'form-label mb-1']) }}
                <div class="input-group">
                    {{ Form::number('discount', 0, ['id' => 'discount', 'class' => 'form-control', 'oninput' => "validity.valid||(value=value.replace(/[e\+\-]/gi,''))", 'min' => '0', 'step' => '.01', 'pattern' => "^\d*(\.\d{0,2})?$"]) }}
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                <span>{{ getCurrencySymbol() }}</span> <span id="quoteTotal" class="price">0</span>
                            </td>
                        </tr>
                        <tr>
                            <td class="ps-0">{{ __('messages.quote.discount') . ':' }}</td>
                            <td class="text-gray-900 text-end pe-0">
                                <span>{{ getCurrencySymbol() }}</span> <span id="quoteDiscountAmount">0</span>
                            </td>
                        </tr>
                        <tr>
                            <td class="ps-0">{{ __('messages.quote.total') . ':' }}</td>
                            <td class="text-gray-900 text-end pe-0">
                                <span>{{ getCurrencySymbol() }}</span> <span id="quoteFinalAmount">0</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <div class="float-end mt-4">
        <div class="form-group col-sm-12">
            <button type="button" name="draft" class="btn btn-primary mx-3" id="saveAsDraftQuote" data-status="0" value="0">
                {{ __('messages.common.save') }}
            </button>
            <a href="{{ route('quotes.index') }}" class="btn btn-secondary btn-active-light-primary">
